...
export async function publishToMainSite(item) {
    ...
    try {
        ...
        const publishDate = item.publishedAt ? new Date(item.publishedAt).toISOString().split('T')[0] : new Date().toISOString().split('T')[0];
        const sourceAttribution = `<p style="color: #666; font-size: 14px; margin-bottom: 20px; padding: 10px; background: #f5f5f5; border-left: 3px solid #007bff;">📰 <strong>출처:</strong> ${item.source || 'Unknown'} | <strong>원문 날짜:</strong> ${publishDate}${item.originalUrl ? ` | <a href="${item.originalUrl}" target="_blank" rel="noopener noreferrer">원문 보기</a>` : ''}</p>`;
